...
import { DropdownContextType,DropdownMenuContentProps,DropdownMenuItemProps,DropdownMenuProps,DropdownMenuTriggerProps } from "./dropdown.types"
  ...
  const setOpen = onOpenChange ?? setUncontrolledOpen
   return(
    <DropdownContext.Provider value={{open,setOpen}}>
       <div className={clsx("relative inline-block  py-2",className)}{...props} >
         {children}
       </div>
...
export const DropdownMenuTrigger=({children,className,asChild,...props}:DropdownMenuTriggerProps)=>{
 ...
 return (
 <div className={clsx("px-2 py-2 border flex items-center border-zinc-600/30 rounded-md cursor-default overflow-hidden font-semibold  ",className)}
 onClick={handleClick}
  {...props} >
    {children}
...
export const DropdownMenuItem=({children,className,...props}:DropdownMenuItemProps)=>{
   ...
   return(
    ...
    onClick={()=>ctx?.setOpen(false)}
     className={clsx("p-1 m-1   hover:bg-zinc-300/40 dark:hover:bg-zinc-800/30 cursor-default rounded-sm text-sm font-medium  ",className)}{...props} >
       {children}  
    </div>
   )
}